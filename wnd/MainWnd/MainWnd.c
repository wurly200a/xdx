    ...
    default:
        ...
        else if( mainWndData.dxDeviceMode==DX_DEVICE_MODE_DX100 )
        {
            ...
            if( Dx100CtrlOnCommand(LOWORD(wParam),HIWORD(wParam),&ctrlMode,&bExtraMode) )
            {
                if( bExtraMode == TRUE )
                {
                    S_DX100_VOICE_PARAM *dx100voiceParamPtr = Dx100GetVoiceParamPtr();
                    S_DX7_VOICE_PARAM *dx7voiceParamPtr = Dx7GetVoiceParamPtr();

                    copyVoiceParamDx100toDx7(dx100voiceParamPtr,dx7voiceParamPtr);
                    Dx7VoiceParamCopy();
                ...
                else if( ctrlMode == DX100_CTRL_MODE_PATCH )
                {
                    Dx100CtrlModeSet(DX100_CTRL_MODE_PATCH);
                    SomeCtrlGroupDisplay(SOME_CTRL_GROUP_1VOICE);
                }
 ...
 ***************************************/
static void
copyVoiceParamDx100toDx7( S_DX100_VOICE_PARAM *dx100voiceParamPtr,S_DX7_VOICE_PARAM *dx7voiceParamPtr )
{
