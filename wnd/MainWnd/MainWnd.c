    ...
    default:
        if( mainWndData.dxDeviceMode==DX_DEVICE_MODE_DX7 )
        {
            ...
            if( Dx7CtrlOnCommand(LOWORD(wParam),HIWORD(wParam),&ctrlMode) )
            {
                ...
                else if( ctrlMode == DX7_CTRL_MODE_ALL_VOICE )
                {
                    Dx7CtrlModeSet(DX7_CTRL_MODE_ALL_VOICE);
                    SomeCtrlGroupDisplay(SOME_CTRL_GROUP_ALL_VOICE);
                }
        ...
        else if( mainWndData.dxDeviceMode==DX_DEVICE_MODE_DX100 )
        {
            ...
            if( Dx100CtrlOnCommand(LOWORD(wParam),HIWORD(wParam),&ctrlMode,&bExtraMode) )
            {
                if( ctrlMode == DX100_CTRL_MODE_PATCH )
                {
                    if( bExtraMode == TRUE )
                    {
                        S_DX100_VOICE_PARAM *dx100voiceParamPtr = Dx100GetVoiceParamPtr(DX100_VOICE_NUM_MAX);
                        S_DX7_VOICE_PARAM *dx7voiceParamPtr = Dx7GetVoiceParamPtr();

                        DxConvertVoiceParamDx100toDx7(dx100voiceParamPtr,dx7voiceParamPtr);
                        Dx7VoiceParamCopyFromStruct();
                        ...
                        SendMessage( SomeCtrlGetHWND(SOME_CTRL_DEVICE_SELECT), CB_SETCURSEL, 1, (LPARAM)0 ); /* DEVICE: 6 OPERATOR(DX7) */
                    }
                    else
                    {
                        Dx100CtrlModeSet(DX100_CTRL_MODE_PATCH);
                ...
                else if( ctrlMode == DX100_CTRL_MODE_ALL_VOICE )
                {
                    if( bExtraMode == TRUE )
                    {
                        S_DX100_VOICE_PARAM *dx100voiceParamPtr;
                        S_DX7_VOICE_PARAM *dx7voiceParamPtr = Dx7GetVoiceParamPtr();
                        INT i;

                        for(i=0; i<32; i++)
                        {
                            dx100voiceParamPtr = Dx100GetVoiceParamPtr(i);

                            DxConvertVoiceParamDx100toDx7(dx100voiceParamPtr,dx7voiceParamPtr);
                            Dx7VoiceParamCopyFromStruct();
                            Dx7voiceCopyFromOneToAll(i);
                        }
